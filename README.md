...
New games settle one normal Practice Rating receipt using their recorded identity and level. Hints and takebacks still make a game unrated, and beta saves made before this activation remain unrated. Saved games, replays and rematches retain their original opponent identity; policy 5 protects an account that has accepted any of these new identities from an older client overwrite. Version 2.6 is live at the existing hostname with historical opponents enabled. See the [deployment record](docs/verification/roster/deployment/live-deployment.md) and [v2.6.0 release](https://github.com/clauding-lab/chess-prodigy/releases/tag/v2.6.0).
