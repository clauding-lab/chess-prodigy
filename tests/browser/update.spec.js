...
test('a real waiting service worker defers active play and preserves saves when accepted',async({page})=>{
 ...
 try{
  ...
  const oldId=await page.evaluate(()=>JSON.parse(localStorage.getItem('chess-prodigy-state-v1')).game.id);
  version=2;
  ...
  expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('chess-prodigy-state-v1')).game.id)).toBe(oldId);
  ...
  const before=await page.evaluate(()=>JSON.parse(localStorage.getItem('chess-prodigy-state-v1')));
  ...
  const after=await page.evaluate(()=>JSON.parse(localStorage.getItem('chess-prodigy-state-v1')));
