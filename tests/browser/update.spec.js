...
test("a real waiting service worker defers active play and preserves saves when accepted", async ({
  ...
  try {
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () => JSON.parse(localStorage.getItem("chess-prodigy-state-v6")).game.hist.length,
        ),
      )
      .toBe(2);
    const oldId = await page.evaluate(
      () => JSON.parse(localStorage.getItem("chess-prodigy-state-v6")).game.id,
    );
    version = 2;
    await page.evaluate(async () => {
    ...
    expect(
      await page.evaluate(() => JSON.parse(localStorage.getItem("chess-prodigy-state-v6")).game.id),
    ).toBe(oldId);
    ...
    const before = await page.evaluate(() =>
      JSON.parse(localStorage.getItem("chess-prodigy-state-v6")),
    );
