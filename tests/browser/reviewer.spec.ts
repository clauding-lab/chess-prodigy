...
test("native worker opponent scores cannot survive as review, including legacy reload", async ({
  ...
  expect(await page.evaluate(() => Reflect.get(window, "__opponentInjections"))).toBeGreaterThan(0);
  ...
  expect(
    Object.values((await saved(page)).game.evals).every(
      (e: unknown) => (e as { review?: { purpose: string } }).review?.purpose === "neutral-review",
    ),
  ).toBe(true);
});
