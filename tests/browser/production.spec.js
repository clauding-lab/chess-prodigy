...
test("corrupt saves remain untouched during play until recovery is confirmed", async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem("chess-prodigy-state-v6", "broken"));
  await page.goto("/");
  await enterPlay(page);
  await page.getByRole("button", { name: "Start", exact: true }).click();
  expect(await page.evaluate(() => localStorage.getItem("chess-prodigy-state-v6"))).toBe("broken");
