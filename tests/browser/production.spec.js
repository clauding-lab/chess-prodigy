...
test("reload preserves game, rating and theme without replaying engine moves", async ({ page }) => {
  ...
  expect((await saved(page)).game.hist.map((e) => e.san)).toEqual(
    before.game.hist.map((e) => e.san),
  );
...
test("keyboard plays and dialogs trap focus; both themes pass axe checks", async ({
  page,
}, info) => {
  await start(page);
  const e2 = page.getByRole("button", { name: "e2, white pawn" });
  await e2.focus();
  ...
  for (const theme of ["wood", "dark"]) {
    await page
      .getByRole("button", { name: theme === "wood" ? "Wooden board" : "Dark board", exact: true })
      .click();
...
test("Strong search keeps the interface responsive and restart rejects old results", async ({
  page,
}, info) => {
  ...
  await page.getByRole("button", { name: "a2, white pawn" }).click();
  await page.getByRole("button", { name: "a3, empty" }).click();
  ...
  expect(Math.max(...beats)).toBeLessThan(250);
  await info.attach("heartbeat", {
    body: JSON.stringify({
      project: info.project.name,
      maxGapMs: Math.max(...beats),
      samples: beats.length,
    }),
    contentType: "application/json",
  });
...
test("cached app reopens offline and calculates an out-of-book reply", async ({
  page,
  context,
}) => {
  await start(page, { strong: true });
  ...
  await reopened
    .getByRole("dialog", { name: "New game", exact: true })
  ...
  await reopened
    .getByRole("dialog")
    .getByRole("button", { name: "Review game", exact: true })
    .click();
...
test("corrupt saves remain untouched during play until recovery is confirmed", async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem("chess-prodigy-state-v3", "broken"));
  await page.goto("/");
  await enterPlay(page);
  await page.getByRole("button", { name: "Start", exact: true }).click();
  expect(await page.evaluate(() => localStorage.getItem("chess-prodigy-state-v3"))).toBe("broken");
