...
function promotionGame(now:number){
 ...
 s=reduceSession(s,{type:'new',setup:{playerColor:'w',level:'club',time:'5+0'},now,id:'promotion-game'});
 for(const san of ['a4','h5','a5','h4','a6','h3','axb7','hxg2']){
  const move=legalMoves(s.game.st).find(m=>sanFor(s.game.st,m,applyMove(s.game.st,m))===san);
...
test('keyboard chooses an underpromotion from a legally replayed saved game',async({page})=>{
 ...
 await page.addInitScript(s=>localStorage.setItem('chess-prodigy-state-v1',JSON.stringify(s)),session);
 await page.goto('/');
 ...
 await page.getByRole('button',{name:'Promote to knight'}).focus();await page.keyboard.press('Enter');
...
test('promotion picker closes on timeout and never commits a late promotion',async({page})=>{
 const now=Date.now();const session=reduceSession(promotionGame(now-299000),{type:'tick',now});
 ...
 expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('chess-prodigy-state-v1')!).game.hist.length)).toBe(8);
});
