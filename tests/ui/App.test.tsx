...
it("starts, plays a legal move, receives an engine reply and preserves theme across reload", async () => {
  ...
  expect(JSON.parse(localStorage.getItem("chess-prodigy-state-v4")!).game.hist).toHaveLength(2);
...
it("keeps timed computer replies running while the player views friend games", async () => {
  ...
  const saved = JSON.parse(localStorage.getItem("chess-prodigy-state-v4")!);
...
it.each([1400, 1500.25])(
  ...
  async (value) => {
    ...
    const session = JSON.parse(localStorage.getItem("chess-prodigy-state-v4")!);
    ...
    expect(JSON.parse(localStorage.getItem("chess-prodigy-state-v4")!).rating.rating).toBe(value);
    fireEvent.click(screen.getByRole("button", { name: "New game", exact: true }));
    expect(
      screen.getByText(
        value === 1400
