 ...
 * Run: Node22 --import tsx THIS_FILE RUN_ROOT OUTPUT_JSON [--self-test]
 * RUN_ROOT contains frozen-source/, frozen-source.json, frozen archive and evidence/.
 */
...
const COMMIT = "adfc0060635fc5d4ee7dc8103dac3722c732408c";
const ARCHIVE_HASH = "ac4f96ddd183bb7316f1ee29627274ae2908304c8a85a2dea66cb5b666dac50c";
const PROTOCOL = "chigorin-plans-paired-v1";
...
const IDENTITY = {
  id: "chigorin", version: 1, engine: "chigorin-plans-v1", randomPolicy: "seeded-per-ply-v1",
};
...
function sourceAudit(root: string) {
  const archive = join(root, "frozen-source-adfc006.tar");
  const bytes = readFileSync(archive);
  assert.equal(hash(bytes), ARCHIVE_HASH, "Frozen archive hash");
  assert.equal(readFileSync(join(root, "frozen-archive.sha256"), "utf8").split(/\s+/)[0], ARCHIVE_HASH);
  assert.equal(execFileSync("git", ["get-tar-commit-id"], { input: bytes.subarray(0, 1024), encoding: "utf8" }).trim(), COMMIT);
  const inventory = json(join(root, "frozen-source.json"));
  const names = Object.keys(inventory).sort();
  assert.equal(names.length, 331, "Frozen file count");
  ...
  for (const file of names) {
    ...
    assert.equal(hash(readFileSync(join(root, "frozen-source", file))), inventory[file], `Frozen bytes: ${file}`);
  }
  function walk(relative = ""): string[] {
    return readdirSync(join(root, "frozen-source", relative), { withFileTypes: true }).flatMap((entry) => {
  ...
  return { inventory, archiveSha256: ARCHIVE_HASH, inventorySha256: hash(readFileSync(join(root, "frozen-source.json"))), files: names.length };
}
...
function gameAudit(board: Board, game: Json, level: Level, pair: number, color: "w" | "b") {
  ...
  assert.ok(number(game.elapsedMs, "elapsed") + 1e-6 >= number(game.morphyMs, "Chigorin time") + number(game.classicMs, "Classic time"), "Engine timings exceed total elapsed time");
...
function logsAudit(root: string, level: Level, files: Map<string, Json>, checkpoints: number[], scores: Score[]) {
  const workerPath = join(root, `${level}-workers.log`);
  const workerText = readFileSync(workerPath, "utf8");
  const lines = workerText.trim().split("\n").filter(Boolean).map((line) => object(JSON.parse(line), "worker log"));
  ...
  const hashes: Record<string, string> = { [`${level}-workers.log`]: hash(workerText) };
  for (const checkpoint of checkpoints) {
    const filename = `${level}-${checkpoint}-verify.log`;
...
function selfTests(board: Board) {
  ...
  for (const mutation of [{ score: 1 }, { fen: "wrong" }, { seed: 9 }, { moves: [...history, "a3"] }, { elapsedMs: 1 }])
  ...
  return "passed: estimator symmetry/bounds/unresolved, legal mate/repetition, tampered score/FEN/seed/post-terminal/timing, legal and pinned en-passant";
}

async function main() {
  const [rootText, outputText, mode] = process.argv.slice(2);
  assert.ok(rootText && outputText && (mode === undefined || mode === "--self-test"), "Usage: audit.ts RUN_ROOT OUTPUT_JSON [--self-test]");
  const root = resolve(rootText), output = resolve(outputText);
  const frozen = sourceAudit(root);
  const board: Board = await import(pathToFileURL(join(root, "frozen-source/src/engine/board.ts")).href);
  ...
  for (const level of LEVELS) {
    ...
    let plies = 0, minPlies = Infinity, maxPlies = 0, elapsedMs = 0, chigorinMs = 0, classicMs = 0;
    for (let pair = 0; pair < pairCount; pair++) for (const color of ["w", "b"] as const) {
      ...
      elapsedMs += number(game.elapsedMs, filename); chigorinMs += number(game.morphyMs, filename); classicMs += number(game.classicMs, filename);
    }
    ...
    audited[level] = { checkpoints: summaries, acceptedFixedRating: final.fixedRating, reasons, plies, minPlies, maxPlies, elapsedMs, chigorinMs, classicMs };
  ...
  for (const [file, expected] of Object.entries(frozen.inventory))
    assert.equal(hash(readFileSync(join(root, "frozen-source", file))), expected, `Source changed during audit: ${file}`);
  const result = {
    ...
    frozenFilesVerified: frozen.files, protocol: PROTOCOL, selfTest, eligibleForRatedIntegration: allEligible,
