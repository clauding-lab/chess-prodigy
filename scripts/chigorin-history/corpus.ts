...
export type ChigorinBook = Record<string, Array<[uci: string, count: number]>>;
...
function gameId(chigorinColor: Color, moves: string[]): string {
  return createHash("sha256")
    .update(`${chigorinColor}\n${moves.join(" ")}`)
    .digest("hex");
}

export function parseChigorinPgn(text: string): {
  games: ChigorinGame[];
  excluded: Array<{ index: number; reason: string }>;
} {
  const games: ChigorinGame[] = [];
  ...
  for (const [recordIndex, raw] of splitRecords(text).entries()) {
    ...
    if (
      Object.entries(tags).some(
        ([key, value]) =>
          ...
          /\b(consultation|consulting|team)\b/i.test(value),
      )
    ) {
    ...
    const chigorinColor =
      tags.White === "Chigorin, Mikhail" ? "w" : tags.Black === "Chigorin, Mikhail" ? "b" : null;
    if (!chigorinColor) {
      reject('player name is not exactly "Chigorin, Mikhail"');
      continue;
    }
    ...
    const id = gameId(chigorinColor, moves);
    if (seen.has(id)) {
...
export function replayChigorinGame(game: ChigorinGame): Move[] {
  let position = START();
  return game.moves.map((san, index) => {
    const move = resolveMove(position, san);
    if (!move) throw new Error(`Chigorin game ${game.id} is illegal at ply ${index + 1}: ${san}`);
    position = applyMove(position, move);
    return move;
  });
}

export function buildChigorinBook(games: ChigorinGame[]): ChigorinBook {
  const counts = new Map<string, Map<string, number>>();
  const fullKeys = new Map<string, string>();
  for (const game of games) {
    let position = START();
    for (const move of replayChigorinGame(game)) {
      if (position.turn === game.chigorinColor) {
        const key = chigorinBookKey(position);
...
export function assertChigorinBookLegal(book: ChigorinBook, games: ChigorinGame[]): void {
  const positions = new Map<string, Position>();
  for (const game of games) {
    let position = START();
    for (const move of replayChigorinGame(game)) {
      if (position.turn === game.chigorinColor) positions.set(chigorinBookKey(position), position);
      position = applyMove(position, move);
    }
  }
  ...
  for (const [key, continuations] of Object.entries(book)) {
    ...
    if (!position) throw new Error("Chigorin book contains a position absent from the corpus");
    ...
    for (const [uci, count] of continuations) {
      if (!legalUci.has(uci)) throw new Error(`Chigorin book contains illegal continuation ${uci}`);
      if (!Number.isSafeInteger(count) || count < 1)
        throw new Error(`Chigorin book contains invalid count for ${uci}`);
    }
  }
}
